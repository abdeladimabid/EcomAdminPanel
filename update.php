...
if(isset($_POST['update'])) {
    // $query3 = "UPDATE items SET name= ".$_POST['name'].",description=".$_POST['desc'].",price=".$_POST['prix'].",categorie=".$_POST['cat']." WHERE item_id=".$id.";";
...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["uploadedfile"]["tmp_name"], $target_file)) {
		...
		<div style='text-align:center' class='alert alert-success' role='alert'>
          The file ". basename( $_FILES["uploadedfile"]["name"]). ". has been uploaded. ".$message."</div>";
          
          unlink($row1['img']);
		
		$url= "upload/".basename( $_FILES["uploadedfile"]["name"]);
		
		$query3 = "UPDATE items SET name='$name',description='$desc',price=$price,cat_id=$cat, img='$url' WHERE item_id=$id";
                ...
                <div class="slam1">
                    <form id="update_form" method="post" enctype="multipart/form-data">
                        ...
                        <div class='form-group'>
                            ...
                            <input type='Name' class='form-control' name='name' id='exampleFormControlInput1' value='<?php echo $row1['name'] ?>'>
                        </div>
                        <div class='form-group'>
                            <label for='Description'>Description</label>
                            <input type='text' class='form-control' id='exampleFormControlInput1' name='desc' value='<?php echo $row1['description'] ?>'>
                        </div>
                        <div class='form-group'>
                            <label for='Prix'>Prix: (en DH)</label>
                            <input type='number' class='form-control' id='exampleFormControlInput1' name='prix' value='<?php echo $row1['price'] ?>'>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="exampleFormControlSelect1" name="cat">
                                ...
                                if ($result = mysqli_query($con, $query2)) {
                                    while ($row = mysqli_fetch_array($result)) {
                                        echo "<option id='cat" . $row['cat_id'] . "' value='" . $row['cat_id'] . "'>" . $row['name'] . "</option>";
                                    }
                                }
                                ?>
                        ...
                        <div class="input">
                        <br><label for="avatar">Sélectionner une image:</label>
                            <input type="file" id="avatar" name="uploadedfile" accept="image/png, image/jpeg">
                        </div>
                        <div class=" button">
                            <button type="submit" class="btn btn-success btn-lg" name="update"><i class='fas fa-pen'></i>&nbsp;&nbsp;Modifier</button>
